...
Amedei has the highest average rating when only looking at companies at with 10+ entries. They are the only company with a rating of 5 which could be skewing their data. 
```{r}
library(tidyverse)
library(dplyr)
companies = count(choco,"Company")
companies_greater = filter(companies, freq > 10)
